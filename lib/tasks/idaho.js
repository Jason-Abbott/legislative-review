...
function parseBillListPage(dom) {
	...
	// limit scrape requests to some [max] simultaneous
	let queue = {
		max: 10,
		current: 0,
		pending: [],
		...
		// method called when a scrape and parse completes
		next() {
			if (this.current < this.max) {
				let fn = this.pending.pop();
				fn(this.next.bind(this));
				this.current++;
			}
		}
	...
	dom('tr[id^="bill"] a:first-child').each((i, el) => {
		queue.run(callback => { scrapeBillPage(url + el.attribs['href'], callback); });
	});
}

function scrapeBillPage(url, callback) {
	request.get(url).end((err, res) => {
		if (err == null && res.statusCode == 200) {
			parseBillPage(cheerio.load(res.text), callback);
		} else {
...
function parseBillPage(dom, callback) {
	//let dbList = db.child('bills/' + year);
	callback();
}
