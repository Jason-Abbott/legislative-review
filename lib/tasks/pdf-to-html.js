...
function isUnderlined(el, page) { return hasLine(el, page, -0.1); }
function isStricken(el, page) { return hasLine(el, page, 0.2); }

function hasLine(el, page, offset = 0) {
	// factor to convert all units to page units
	const factor = page.Height / layout.height;
	const xTolerance = 0.3;
	const yTolerance = layout.line.tolerance / factor;
	const y = el.y + (offset / factor);
	...
	return page.HLines.findIndex(line =>
		...
		(line.y <= y + yTolerance || line.y > y - yTolerance) && 
