...
// layout dimensions in standard units
const layout = {
	height: 11,
	width: 8.5,
	space: 0.044,
	...
	let html = pdf.formImage.Pages
		.map(p => p.Texts
			...
			.reduce((lines, el) => {
				...
				if (el.left < layout.margin.number && /^\d+$/.test(el.text)) {
					...
					// track maximum word extents to indentify indents and wrapping
					if (el.right > line.right) { line.right = r(el.right); }
					if (el.left < line.left) { line.left = r(el.left); }
					// don't push line numbers into the word list
					if (el.underline) {
		...
		.map((line, i, lines) => {
			if (i > 0) {
				...
				let indented = line.left >= layout.margin.left + layout.indent
								&& line.right <= (layout.width - layout.margin.right) - layout.indent;
				let doubleSpace = {
				...
				if (doubleSpace.before) {
					...
					(line.right != blockQuote.right || !doubleSpace.after)) { blockQuote.start = 0; }
			}
			return line;
		})
		.map(line => {
			if ((line.blockQuoteRight > 0 && line.right < line.blockQuoteRight) ||
				(line.blockQuoteRight == 0 && line.right < layout.width - layout.margin.right)) {
		...
		.replace(/ *\n */g, '\n')
		// hyphenated words (could be inserted or deleted)
