...
module.exports = function(pdf) {
	...
	let html = pdf.formImage.Pages
		...
		.map(line => {
			...
			// or if the last words ends with a hypen add line-break for easier identification later
			if (line.right < layout.line.rightMargin || /\- *$/.test(line.words[line.words.length - 1])) {
				line.words.push('\n');
			}
			previousLine = line;
			return line;
