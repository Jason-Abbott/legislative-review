...
module.exports = {
	...
	indexes(text) {
		// text followed by at least four periods then more text at the line end
		let rowPattern = '\\n(.+?(?!\\.{4})) +\\.{4,}(.+)\\b';
		// at least two rows
		let table = new RegExp('((?:' + rowPattern + '){2,})\\n', 'g');
		let row = new RegExp(rowPattern, 'g');

		return text.replace(table, (match, capture) => {
			row.lastIndex = 0;
			return '<table class="index">' +	capture.replace(row, (m, col1, col2) =>
